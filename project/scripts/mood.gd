class_name MoodKit
extends Node
# Three lighting/color moods, tweened. A/X (or 1/2/3 on desktop) cycles.
...
func apply(name: String, instant: bool) -> void:
	...
	if instant:
		...
		main.firefly_mat.emission = p.firefly
	else:
		...
		tw.tween_property(main.firefly_mat, "emission", p.firefly, dur)
